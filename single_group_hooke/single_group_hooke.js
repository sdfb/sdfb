...
complexityButtons.on('change', function () {
    ...
    if (complexity == 0) {
      d3.select("#complexityLabel").text("Network Complexity: Group members only ");
      currentNodes = graph.nodes.filter(function(d) { if (d.distance == 0) {return d;}; });
      currentLinks = graph.links.filter(function(l) { if (l.source.distance == 0 && l.target.distance == 0) {return l;}; });
      update(currentNodes, currentLinks, threshold);
    }

    if (complexity == 1) {
      d3.select("#complexityLabel").text("Network Complexity: Group members + 1-degree Connections ");
      currentNodes = graph.nodes;
      currentLinks = graph.links;
      update(currentNodes, currentLinks, threshold); }
