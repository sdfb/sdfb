class LargeData < ActiveRecord::Base
	...
	def file_formatted_correctly
		...
		#change to return errors instead of false
		...
		if (self.table_content_type == "Relationship")
			first_person_index = header_row.index("first_name 1")
			irst_person_index = header_row.index("first_name 2")
			second_person_index = header_row.index("last_name 1")
			second_person_index = header_row.index("last_name 2")
			original_certainty_index = header_row.index("orginal_certainty")
			return false if (first_person_index.nil? || second_person_index.nil? ||
				original_certainty_index.nil?)
			body_rows.each do |row|
				return false if row[first_person_index].nil?
				return false if row[second_person_index].nil?
				return false if row[original_certainty_index].nil?
				return false if row[first_person_index] == row[second_person_index]				
			end
			...
			description_index = header_row
			.index("description")
			first_name_index = header_row.index("first_name")
			last_name_index = header_row.index("last_name")
			return false if group_name_index.nil? || description_index.nil? || first_name_index.nil? || last_name_index.nil?
			body_rows.each do |row|
				return false if row[group_name_index].nil? || row[description_index].nil?
				return false if row[first_name_index].nil?
				return false if row[last_name_index].nil?
				return false if row[group_name_index].length < 3
			end
	...
	def display_duplicates_in_db
		...
		if (self.table_content_type == "Group")
			...
			group_hash = {} 
			0.upto(data_rows.count) do |row_index| 
				if rows_checked.include?(row_index) 
					#Do Nothing
				else
					match_array = [] #array of groups similar to the one at row index
					match = Group.find_by(name: row[first_row["name"]], description: row[first_row["description"]])
					match_array << match if !match.nil?

					match = Group.find_by(name: row[first_row["name"]], start_year: row[first_row["start_year"]], end_year: row[first_row["end_year"]])
					match_array << match if !match.nil?

					match = Group.find_by(name: row[first_row["name"]])
					db_person_list = []
					...
					new_hash[row_index] = match_array #still maps matches to first rownumber with that group name
				end
			end
			...
			CSV.read(file_path).each do |row| 
				...
				if row_num == 0
					...
					for heading in row 
						...
						if ["odnb_id", "display_name", "first_name", "last_name", "ODNB 1", "ODNB 2", "name"].include?(heading)			
					...
					if (self.table_content_type == "Person")
						...
						person_two = Person.find_by(first_name: row[first_row["first_name 1"]], last_name: row[first_row["last_name 1"]]) 
						if person_one.nil? || person2.nil?
						 	#######break
						else
	...
	def merge_and_remove_duplicates(merge_hash) 
		...
		merge_hash.keys.each do |row_number|
			...
			data_file.delete_at(row_number) if match.nil?
		...
		if (self.table_content_type == "Person") 
			...
			data_object.each do |row|
				0.upto(row.count) do |entry_index|
					data_hash[first_row[entry_index]] = row[entry_index]
			...
			data_object.each do |row|
				...
				#person2_index
				new_hash[:person2_index] = Person.find_by(first_name: row[headers[:fn2_index]], last_name: row[headers[:ln2_index]]).id
				#original_certainty
			...
			data_file.each do |row|
				key = people_lists[row[headers[:n_index]]]
				people_lists[key] = [] if !people_lists.has_key?(key)
				people_lists[key] = Person.find_by(first_name: row[headers[:fn_index]], last_name: row[headers[:ln_index]])
			end
			groups_added = []
			data_object.each do |row| 
	...
	def meet_validations(data, content_type)
		...
		if (content_type == "Person")
			...
			data.gender = "unknown" if data.gender == nil
			...
			if data.start_year.present?
				...
				data.start_date_type = "IN" if !data.start_year.nil? && (data.birth_year_type == nil || !date_types.include?(data.start_date_type))
			...
			data.start_year = nil if data.start_year < 1400 
			data.start_year = nil if data.start_year > 1800
			data.start_date_type = "IN" if data.birth_year_type == nil || !date_types.include?(data.start_date_type)
			end
			if data.end_year.present?
			data.end_date_type = "IN" if data.end_date_type == nil || !date_types.include?(data.end_date_type)
			data.end_year = nil if data.end_year < 1400 
			data.end_year = nil if data.end_year > 1800
			end
		end
