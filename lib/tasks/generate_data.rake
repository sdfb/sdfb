namespace :db do
  task :generate_data => :environment do
    ...
    outFile << "COPY people (id, odnb_id, prefix, first_name, last_name, suffix, title, gender, search_names_all, birth_year_type, ext_birth_year, alt_birth_year, death_year_type, ext_death_year, alt_death_year, historical_significance, rel_sum, is_approved, created_by, approved_by, approved_on,created_at,updated_at) FROM stdin;\n"
    ...
    inFile.each { |line|
      ...
      data.each { |person|
        ...
        outFile << "#{personData[0].to_i}\t#{(personData[2] != "NA") ? personData[2].to_i : 0 }\t#{personData[3]}\t#{personData[4]}\t#{ personData[5]}\t#{ personData[6]}\t#{ personData[7]}\t#{"unknown"}\t#{ personData[9] }\t#{personData[19]}\t#{personData[20]}\t#{personData[21]}\t#{personData[22]}\t#{personData[23]}\t#{personData[24]}\t#{personData[26]}\t#{[]}\t#{true}\t#{uid}\t#{uid}\t#{cur_time}\t#{cur_time}\t#{cur_time}\n"
    ...
    outFile << "COPY relationships (id, person1_index, person2_index, created_by, approved_by, is_approved, approved_on, max_certainty, original_certainty, edge_birthdate_certainty, created_at,updated_at) FROM stdin;\n"
    ...
    files.each { |current_file_name|
      ...
      inFile.each { |line|
        ...
        data.each { |relationship|
          ...
          rec = { id: (next_id+=1), from_id: relData[0].to_i+10000000, to_id: relData[1].to_i+10000000,created_by: uid, approved_by: uid, is_approved: true, approved_on: current_time, max_certainty:(relData[2].to_f*100.00).to_i, original_certainty: (relData[2].to_f*100.00).to_i, edge_birthdate_certainty: (relData[3].to_f*100.00).to_i }
